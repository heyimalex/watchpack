...
class DirectWatcher {
	constructor(filePath) {
		console.log(`DirectWatcher.ctor: ${filePath}`);
	...
	remove(watcher) {
		...
		if (this.watchers.size === 0) {
			...
			watcherCount--;
			if (this.watcher) {
				console.log(`DirectWatcher.close: ${this.filePath}`)
				this.watcher.close();
			}
		}
	}
...
class RecursiveWatcher {
	constructor(rootPath) {
		console.log(`RecursiveWatcher.ctor: ${rootPath}`);
	...
	remove(watcher) {
		...
		if (this.mapWatcherToPath.size === 0) {
			...
			watcherCount--;
			if (this.watcher) {
				console.log(`RecursiveWatcher.close: ${this.rootPath}`)
				this.watcher.close();
			}
			if (recursiveWatcherLogging) {
				process.stderr.write(
